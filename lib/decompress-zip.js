...
function IsFileException(str) {
    return ~FileExceptions.indexOf(str.toLowerCase());
}

function FilterRemoveEmpties(str) {
    return !!str;
}
...
DecompressZip.prototype.extract = function (options) {
    ...
    options.stripCommonPaths = !!options.stripCommonPaths;
        ...
        if (options.stripCommonPaths) {
            ...
            for (; i < len; i++) {
                ...
                if (file.type !== 'Directory') {
                    var filePath = file.path.split('/').filter(FilterRemoveEmpties);

                    // Make sure there is a path and it contains no file exceptions
                    if (filePath.length && !filePath.some(IsFileException)) {
                        if (stripPath === null) {
                            stripPath = filePath;
                        } else if (stripPath.length) {
                            // Iterate through both to find the most common path
                            var commonLen = Math.min(filePath.length, stripPath.length);
                            for (var cpi = 0; cpi < commonLen; cpi++) {
                                if (filePath[cpi] !== stripPath[cpi]) {
                                    // Store the leading common path between the two
                                    stripPath = filePath.slice(0, cpi);
                                    break;
                                }
                            }
                        } else {
                            // Break early no common paths
                            break;
                        }
                    }
                }
            }
            // Store the strip depth
            options.strip = (stripPath && stripPath.length) || 0;
        }
